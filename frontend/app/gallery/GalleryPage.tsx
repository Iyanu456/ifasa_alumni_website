"use client";

import { useMemo, useState } from "react";
import Image from "next/image";
import { X, Download } from "lucide-react";
import { useGalleryQuery } from "../apiServices/queries";

const TABS = ["All", "Events", "Reunions", "Campus", "Workshops", "Other"] as const;

export default function GalleryPage() {
  const [activeTab, setActiveTab] = useState<(typeof TABS)[number]>("All");
  const [activeImage, setActiveImage] = useState<{
    imageUrl: string;
    altText?: string | null;
    category: string;
    title: string;
  } | null>(null);
  const galleryQuery = useGalleryQuery({
    category: activeTab,
    limit: 60,
  });

  const filteredImages = useMemo(
    () => galleryQuery.data?.data ?? [],
    [galleryQuery.data],
  );

  return (
    <main className="bg-[#f8f8f8] min-h-screen pt-[3em] md:pt-[6em]  pb-[4em]">
      <section className="w-[92%] sm:w-[90%] md:w-[80%] mx-auto mb-6">
        <h1 className="text-2xl sm:text-3xl font-semibold mb-2">Gallery</h1>
        <p className="text-gray-600 text-sm sm:text-base">
          Moments from alumni events, reunions, workshops, and campus life.
        </p>
      </section>

      <section className="w-[92%] sm:w-[90%] md:w-[80%] mx-auto mb-6 flex flex-wrap gap-2">
        {TABS.map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`px-4 py-2 rounded-full text-sm font-medium transition ${
              activeTab === tab
                ? "bg-primary text-white"
                : "bg-white text-gray-600 hover:bg-gray-100"
            }`}
          >
            {tab}
          </button>
        ))}
      </section>

      {galleryQuery.isLoading ? (
        <section className="w-[92%] sm:w-[90%] md:w-[80%] mx-auto columns-1 sm:columns-2 md:columns-3 gap-4">
          {Array.from({ length: 8 }).map((_, index) => (
            <div key={index} className="mb-4 h-[260px] rounded-xl bg-gray-100 animate-pulse" />
          ))}
        </section>
      ) : galleryQuery.isError ? (
        <section className="w-[92%] sm:w-[90%] md:w-[80%] mx-auto text-center bg-white rounded-2xl p-10 border border-gray-100 text-red-600">
          Unable to load gallery items right now.
        </section>
      ) : filteredImages.length === 0 ? (
        <section className="w-[92%] sm:w-[90%] md:w-[80%] mx-auto text-center bg-white rounded-2xl p-10 border border-gray-100 text-gray-500">
          No gallery items found for this category.
        </section>
      ) : (
        <section className="w-[92%] sm:w-[90%] md:w-[80%] mx-auto columns-1 sm:columns-2 md:columns-3 gap-4">
          {filteredImages.map((img) => (
            <div
              key={img._id}
              className="group mb-4 break-inside-avoid overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm hover:shadow-md transition"
            >
              <button
                onClick={() =>
                  setActiveImage({
                    imageUrl: img.imageUrl,
                    altText: img.altText,
                    category: img.category,
                    title: img.title,
                  })
                }
                className="relative block w-full aspect-[4/3] overflow-hidden"
                aria-label={`View ${img.altText || img.title}`}
              >
                <Image
                  unoptimized
                  src={img.imageUrl}
                  alt={img.altText || img.title}
                  fill
                  sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"
                  className="object-cover transition-transform duration-300 group-hover:scale-[1.03]"
                />

                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end">
                  <div className="p-3 text-left">
                    <p className="text-white text-sm font-medium leading-snug">
                      {img.altText || img.title}
                    </p>
                    <span className="text-white/80 text-xs">{img.category}</span>
                  </div>
                </div>
              </button>

              <div className="flex items-center justify-between px-3 py-2 text-xs sm:text-sm bg-white gap-4">
                <span className="text-gray-500 line-clamp-1">{img.altText || img.title}</span>

                <a
                  href={img.imageUrl}
                  download
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1 text-primary hover:underline"
                  onClick={(e) => e.stopPropagation()}
                >
                  <Download size={14} />
                  Download
                </a>
              </div>
            </div>
          ))}
        </section>
      )}

      {activeImage && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <button
            onClick={() => setActiveImage(null)}
            className="absolute top-4 right-4 text-white hover:opacity-80"
            aria-label="Close"
          >
            <X size={28} />
          </button>

          <div className="relative max-w-5xl w-full">
            <Image
              unoptimized
              src={activeImage.imageUrl}
              alt={activeImage.altText || activeImage.title}
              width={1200}
              height={800}
              className="w-full h-auto rounded-lg object-contain"
            />

            <div className="flex items-center justify-between mt-3 text-sm text-gray-200 gap-4">
              <p>
                {activeImage.altText || activeImage.title} • {activeImage.category}
              </p>

              <a
                href={activeImage.imageUrl}
                download
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1 text-white hover:underline"
              >
                <Download size={16} />
                Download
              </a>
            </div>
          </div>
        </div>
      )}
    </main>
  );
}
