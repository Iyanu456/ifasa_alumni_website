...
export const updateSettingsValidation = [
  ...
  body("accountName").optional().trim().isLength({ min: 2, max: 120 }),
  body("donationAccountNumber").optional().trim().isLength({ min: 5, max: 40 }),
  body("donationBankName").optional().trim().isLength({ min: 2, max: 120 }),
  body("footerText").optional().trim().isLength({ max: 300 }),
